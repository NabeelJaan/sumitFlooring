...
wp_enqueue_script( 'app', get_template_directory_uri().'/assets/js/jquery.sticky.js', array(), false, true );
...
if( !function_exists('sc_change_custom_post_type_slug') ):
    ...
    function sc_change_custom_post_type_slug( $args, $post_type ) {
        ...
        if ( 'products' === $post_type ) {
            $args['rewrite']['slug'] = 'product/detail';
        }   
             
 		if ( 'video' === $post_type ) {
            $args['rewrite']['slug'] = 'product_videos/video';
        }    

        if ( 'ourteam' === $post_type ) {
...
function setup_phpmailer_init( $phpmailer ) {
    ...
    $phpmailer->Username = 'summitintlflooringwebsite@gmail.com'; // your SMTP username
    $phpmailer->Password = 'CIPRCOM#01'; // your SMTP password
    $phpmailer->SMTPAuth = true;
    $phpmailer->SMTPSecure = 'tls'; // preferable but optional
    $phpmailer->From       = 'info@summit-flooring.com';
    $phpmailer->FromName   = 'summit-flooring';
    $phpmailer->IsSMTP();
